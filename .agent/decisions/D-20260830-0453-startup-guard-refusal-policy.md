...
生存記録の読み取りと `touchRunnerRecord` の書き込みの間に排他が無いため、2 プロセスが完全に同時刻に
起動して両方が `no-record` を読むと、理論上はどちらも起動を許してしまう。実際に報告された事故は
「先発が既に動作中のところへ後発を起動する」形であり、そちらは確実に防げる。この隙間を埋めるには
ロックファイルが必要だが、上記のとおり異常終了時のロック残骸という別の詰みを招くため採らない。
