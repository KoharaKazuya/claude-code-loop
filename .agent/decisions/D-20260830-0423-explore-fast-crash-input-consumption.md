...
**フラグを永続化しない理由。** `lastExploreFastCrashed` はプロセス内変数である。再起動を
またぐ抑制は `state.lastExploreAt` 由来の `exploreDue` が担うため二重に持つ必要がなく、人間が
原因(claude の設定ミス等)を直して `ccloop run` を打ち直したときは即座に再探索できるほうがよい。
