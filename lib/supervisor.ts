...
function runningTasksSection(ctx: ExploreContext): string {
  ...
  return [
    ...
    "実行中のタスクの `.agent/tasks/<id>.md` は編集しない(priority・dependencies・conflicts・",
    "status のいずれも変更しない)。調整が必要な場合は本文にメモを残すか、次回の探索セッションに委ねる。",
    "結果はまだ出ていない前提で判断すること。",
    "",
...
export function buildExplorePrompt(ctx: ExploreContext): string {
  ...
  return [
    ...
    [
      ...
      "2. 1. の回答を含む新しい情報を踏まえて、既存タスクの priority・dependencies・conflicts を",
      "   見直し、タスク全体の消化順序を再検討する。触る見込みのファイルが重なるタスクの組",
      "   (特に変更が集中している大きなファイルを複数タスクが触る場合)には `conflicts` を付け、",
      "   同時に走らないようにする。順序が必要なわけではないので、衝突回避のために",
      "   `dependencies` は張らない。新規タスクを登録するときも同様に判断する。",
      "   見直した場合は理由を該当タスクの note または",
