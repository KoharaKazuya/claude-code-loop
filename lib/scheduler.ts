...
export interface LoopInput {
  now: Date;
  /** once モード(1 セッション実行したら終了する) */
  once: boolean;
  /** このループで完了したセッション数(once モードの終了判定用) */
  completedCount: number;
  /** .agent/STOP による停止指示。null なら停止指示なし */
  stopMode: "session" | "clean" | null;
  /** .agent/ 以外に未コミットの差分があるか(STOP(clean) の停止理由・ログ用) */
   ...
   * once モードの探索判定と、run モードで直前の探索が空振りだったときのクールダウン判定に使う */
  exploreDue: boolean;
  /** このプロセス起動後、探索セッションを一度でも完了させたか(run モードの自動終了判定に使う) */
  exploreDone: boolean;
  /** 直前に完了した探索セッションが新規タスクを 1 件も登録しなかったか(プロセス内で追跡)。
   * true の間は run モードでも exploreDue のクールダウンを課し、空振り探索の即時連鎖を防ぐ */
...
/** rate limit 待機の 1 スライス。長時間待ちでも定期的に STOP を拾い直すため上限を設ける */
export const RATE_LIMIT_SLICE_MS = 60_000;

export const STOP_REASON = {
  session: "STOP (session): セッション境界で停止する(再開時は .agent/STOP を削除)",
  clean: "STOP (clean): 一区切りとして停止する(再開時は .agent/STOP を削除)",
  cleanDirty:
 ...
 *    最大 RATE_LIMIT_SLICE_MS のスライスで待つ。待機中も周回ごとに STOP を拾い直せる。
 *    ただし once で 1 セッション完了済みの場合は待たず 3 の終了判定へ落とす
 *    (once はセッションが rate limit に当たった時点で終了するのが従来の挙動であり、
 *    ここで待つと解除時刻まで何時間も動かないまま居座ってしまう)。
 * 3. once で 1 セッション完了済み → 終了。
 * 4. 人間からの入力が変化した → 既存タスクより先に割り込ませる。triage が有効かつこの入力
 ...
 *    (rate limit(2)と通常のタスク起動(6)の間に位置する: rate limit ほど長くは待たないが、
 *    機械的に起動し続けて失敗を積み増すことは避ける)。
 * 6. 空きスロットと実行可能タスクがある → 空き数だけ起動する。
 * 7. 探索したい → 4 と同様、走っているタスクセッションを待たずに並走させる。
 *    run: このプロセス起動後まだ探索を完了させていない(exploreDone=false)なら探索する。
 *    ただし直前の探索が空振りだった場合(lastExploreYieldedNothing=true)は、
 *    クールダウン(exploreDue)が経過するまで再探索しない(空振り探索の即時連鎖を抑制する)。
 *    once: 従来どおり exploreDue のみで判断する。
 * 8. 実行可能タスクは無いが走っているセッションがある → 完了を待つ。
 * 9. once で実行できるものが何も無い → 終了。
 * 10. run でスヌーズ中のタスクが無い(pendingSnoozeCount === 0)→ 終了(idle-exit)。
 *     ここに到達した時点で「STOP なし・rate limit なし・inputs 変化なし・runnable なし・
 *     実行中なし・探索済み(または explore 無効)」が構造的に成立しているため、探索しても
 *     新しい実行可能タスクは生まれなかったとみなせる。
 * 11. それ以外(スヌーズ中のタスクが残っている)→ アイドル待機(時間が来れば runnable に戻る)。
 */
export function planLoopStep(input: LoopInput): LoopAction {
  const drain: LoopAction = { type: "wait", ms: input.idlePollMs, why: "drain" };

  // 1. 停止指示
  if (input.stopMode !== null) {
  ...
  const onceDone = input.once && input.completedCount >= 1;

  // 2. rate limit 待機
  if (!onceDone && input.rateLimitedUntilMs !== null && input.rateLimitedUntilMs > 0) {
    return { type: "wait", ms: Math.min(input.rateLimitedUntilMs, RATE_LIMIT_SLICE_MS), why: "rate-limit" };
  }

  // 3. once の完了
  if (onceDone) return { type: "stop", reason: STOP_REASON.onceDone };

  // 4. 入力変化による割り込み(triage → 解決しきれなければ探索)
  ...
  // 7. 探索
  // run: 未探索ならクールダウンを待たずに探索する。ただし直前の探索が空振りだった場合は
  //      クールダウン(exploreDue)が経過するまで再探索しない / once: 従来どおり exploreDue
  const exploreWanted = input.once
    ? input.exploreDue
    : !input.exploreDone && (!input.lastExploreYieldedNothing || input.exploreDue);
  ...
  // 9. once で何も実行できない
  if (input.once) return { type: "stop", reason: STOP_REASON.onceIdle };

  // 10. run の自動終了: スヌーズ中のタスクが残っていなければ、探索してもタスクが
  // 生まれなかったとみなして終了する
  if (input.pendingSnoozeCount === 0) {
    return { type: "stop", reason: STOP_REASON.idleExit, cause: "idle-exit" };
  }

  // 11. アイドル待機(スヌーズ中タスクが時間経過で runnable に戻るのを待つ)
