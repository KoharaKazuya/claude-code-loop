...
注意点(自動実行するエージェント向け):
  - run は起動した端末で動き続ける常駐プロセスで、停止するまで終了しない。想定する使い方は
    起動した端末をそのまま開いたままにすることなので、進捗は別の端末から status か watch で見る
    (run 自体の終了を待つ呼び出し方をすると、停止するまで待ち続ける)
...
起動した端末で動き続ける常駐プロセスであり、停止するまで終了しない。その端末はそのまま開いたままに
しておき、進捗は別の端末から \`ccloop status\` か \`ccloop watch\` で見る。
