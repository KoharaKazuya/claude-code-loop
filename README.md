...
```jsonc
{
  "features": {
    ...
    "ghcr.io/koharakazuya/claude-code-loop/ccloop:0.2.0": {}
  }
}
```
...
- 2 回目: 実行中のセッションが返ってきた時点で即終了する(作業途中の差分が残りうる)
- 3 回目: 緊急停止(SIGTERM を送り、猶予後も生きていれば SIGKILL)
- 4 回目: 即 SIGKILL
...
公開済み feature(`ghcr.io/koharakazuya/claude-code-loop/ccloop:0.2.0`)であり、この checkout の
