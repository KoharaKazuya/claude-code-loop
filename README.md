...
Ctrl+C しなくても、実行できるタスクが無くなり新しく探索する理由も無くなった時点で `ccloop run` は
終了する。異常ではなく設計どおりの挙動で、放置しても勝手に再開はしない。ただし次の場合は終了せず続く。

- 時間待ちのタスク(`snoozeUntil` で待機中のもの)が残っている場合は、その時刻が来るまで待つ
- `.agent/GOAL.md` や Human Review の回答など人間の入力、または main の変化があった場合は、
  探索セッションが走って新しいタスクを探す。ただし直前の探索が何も見つけられなかった直後は、
  同じ内容で探索を繰り返さないよう間を置くため、探索せずに終了することがある

終了時に blocked / failed のタスクや未回答の BLOCK が残っていればその件数を知らせるので、
`ccloop status` で内容を確認して対応する。対応後 `ccloop run` を実行し直せばループは再開する。
