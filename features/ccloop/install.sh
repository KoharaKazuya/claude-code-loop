...
#   lib/ と bin/ が同梱されている (ローカル開発でコピー前の状態では失敗する)
#
...
if [ ! -d "${SCRIPT_DIR}/lib" ] || [ ! -d "${SCRIPT_DIR}/bin" ]; then
  echo "error: ${SCRIPT_DIR}/lib または ${SCRIPT_DIR}/bin が見つかりません。" >&2
  echo "       このディレクトリはリリースビルド (.github/workflows/release.yml) が" >&2
  echo "       リポジトリの lib/ と bin/ をコピーして生成する想定のもので、" >&2
