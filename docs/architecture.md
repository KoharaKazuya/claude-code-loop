...
(1 回目: 新規セッション停止 → 実行中の完了待ち、2 回目: 完了待ちのまま即終了、3 回目: SIGTERM →
猶予後 SIGKILL、4 回目: 即 SIGKILL)だけで完結し、`STOP` ファイルのような永続的な停止指示ファイルは
存在しない。
...
- 連番は「既存ファイルの最大番号 + 1」で決まるため、複数の worktree で並行して動くセッション同士が
  同じ番号を採番して衝突しやすい。マージ時にどちらかを改番する必要があり、改番すると本文中の ID 参照
  (`dependencies` / `tasks` / `review` など)も追従して書き換えなければならず、複雑さと取りこぼしの
  リスクを生んでいた。日時を ID に含めることで採番だけでは衝突しなくなり、マージ時の改番ロジック自体を
  廃止できる(同名ファイルの add/add はその場で解決できない実質的な内容衝突として、通常の
  コンフリクトと同じく人間に回す)。
...
- slug は ASCII の名詞句限定(`^[a-z0-9]+(?:-[a-z0-9]+)*$`)にしている。ID はブランチ名
  ...
  安全に使える文字集合に制限している。
- 検討した代替案:
  - 連番 + slug(例: `T-042-rename-agent-files`): 連番部分が並行 worktree 間で衝突する問題は
    解決しない。
  - ランダムな短 ID(例: `T-a3f9c2`): 衝突は避けられるが、`ls` の並び順が作成順と一致せず、
    decisions のローテーション(直近 10 件を残す)が辞書順に依存できなくなる。内容の推測もできない。
  - ID とファイル名を分離する(frontmatter に ID、ファイル名は別体系): ファイル名だけで一意な索引に
    なるという単純さが失われ、実装・運用ともに複雑になる。
