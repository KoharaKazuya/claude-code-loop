...
- アイドル時: 実行可能タスクが無く、前回探索以降に main(タスクのマージ)または入力(GOAL.md /
  Human Review 回答)が変化したか、一度も探索していない。ただし直前の探索が空振り(新規タスクなし)
  なら `explore.minIntervalMs` のクールダウンが経過するまで待つ。
...
- 探索は既存タスクの書き換え・取り下げを行うため、他のセッションと同時に起動すると、古いタスクを
  ...
  新規起動を止める。
- 既存の走行中セッションまでは止めない(旧 drain 方式を採らない)。探索は main の完全な静止
  スナップショットを必要とせず、探索中に入ったマージは次回の探索で吸収されるため。
- トレードオフとして探索中は空き枠が埋まらない。上限は「`minIntervalMs` あたり探索 1 本分の時間 ×
  空いた枠数」で許容している。
