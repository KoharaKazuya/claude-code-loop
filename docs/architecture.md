...
タスクファイルが実在する `agent/<taskId>` だけを除外している。このリポジトリは自分自身で自律運用
されているため、テスト実行中にも別セッションが正当な作業ブランチを作る。それを事故と区別するには
「対応するタスクが存在するか」を見るのが最も確実で、これは Supervisor の孤児ブランチ判定と同じ基準
である。減ったブランチを無視するのも同じ理由で、こちらは孤児ブランチ回収や他セッションの後片付けが
正常に働いた結果である。
