...
describe("checkVersion", () => {
  it("3 箇所が一致していれば ok になる", () => {
    writeFixture({ packageVersion: "0.1.0", featureVersion: "0.1.0", readmeVersions: ["0.1.0"] });
  ...
  it("feature の version が package と異なれば不一致として各値を報告する", () => {
    writeFixture({ packageVersion: "0.1.0", featureVersion: "0.2.0", readmeVersions: ["0.1.0"] });
  ...
  it("README に ccloop feature 参照が無ければ不一致として報告する", () => {
    writeFixture({ packageVersion: "0.1.0", featureVersion: "0.1.0", readmeVersions: [] });
...
describe("syncVersion", () => {
  it("package.json の version に feature / README を同期し、変更ファイルを報告する", () => {
    writeFixture({ packageVersion: "0.2.0", featureVersion: "0.1.0", readmeVersions: ["0.1.0", "0.1.0"] });
